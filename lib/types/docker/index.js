...
import { DockerError } from "./types.js";
export { DockerError } from "./types.js";
/**
...
export class DockerRuntime extends Service {
    ...
    registerProvider(provider) {
        if (this.providers.has(provider.id)) {
            throw new DockerError(`docker provider "${provider.id}" is already registered`, 'DOCKER_PROVIDER_DUPLICATE');
        }
    ...
    async select() {
        ...
        if (configuredId !== undefined && configuredId.length > 0) {
            ...
            if (configured === undefined) {
                throw new DockerError(`configured docker provider "${configuredId}" is not registered`, 'DOCKER_PROVIDER_CONFIGURED_MISSING');
            }
            if (!await configured.available()) {
                throw new DockerError(`configured docker provider "${configuredId}" cannot reach its engine`, 'DOCKER_PROVIDER_CONFIGURED_UNAVAILABLE');
            }
            return configured;
        }
        const usable = [];
        ...
        const [only] = usable;
        if (only === undefined) {
            throw new DockerError('no usable docker provider is registered', 'DOCKER_PROVIDER_UNAVAILABLE');
        }
        if (usable.length > 1) {
            throw new DockerError(`several usable docker providers (${usable.map(p => p.id).join(', ')}); set the seam's provider`, 'DOCKER_PROVIDER_AMBIGUOUS');
        }
        return only;
    }
    /**
    ...
    async startEngine(signal) {
        ...
        if (provider?.startEngine === undefined) {
            throw new DockerError('no registered docker provider can start a local engine', 'DOCKER_ENGINE_UNMANAGEABLE');
        }
    ...
    async installEngine(signal) {
        ...
        if (provider?.installEngine === undefined) {
            throw new DockerError('no registered docker provider can install a local engine', 'DOCKER_ENGINE_UNMANAGEABLE');
        }
    ...
    async removeContainer(request, signal) {
        ...
        if (provider.removeContainer === undefined) {
            throw new DockerError(`docker provider "${provider.id}" cannot remove containers`, 'DOCKER_UNSUPPORTED');
        }
    ...
    async removeImage(request, signal) {
        ...
        if (provider.removeImage === undefined) {
            throw new DockerError(`docker provider "${provider.id}" cannot remove images`, 'DOCKER_UNSUPPORTED');
        }
