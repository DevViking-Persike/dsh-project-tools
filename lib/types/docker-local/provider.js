...
function labelValue(labels, key) {
    ...
    for (const entry of labels.split(',')) {
        ...
        if (eq < 1)
            continue;
        if (entry.slice(0, eq) === key)
            return entry.slice(eq + 1) || undefined;
    }
    return undefined;
}
/**
...
export class LocalDockerProvider {
     ...
     */
    async cli(args, timeoutMs, signal) {
     ...
     */
    async run(args, timeoutMs, signal) {
        ...
        for (let retry = 0; readOnly && retry < this.limits.inspectConnectionRetries; retry++) {
            if (result.exitCode === 0 || !/grpc: the client connection is closing/i.test(result.stderr))
                break;
            signal?.throwIfAborted();
        ...
        if (result.exitCode !== 0) {
            ...
            throw new DockerError(`docker ${args.join(' ')} failed: ${detail || `exit ${String(result.exitCode)}`}`, missing ? 'DOCKER_NOT_FOUND' : 'DOCKER_ENGINE_FAILED');
        }
        return result;
    }
    async available() {
     ...
     */
    async tool(argv, timeoutMs, signal) {
        const timeout = AbortSignal.timeout(timeoutMs);
        const combined = signal === undefined ? timeout : AbortSignal.any([signal, timeout]);
        const collect = { maxBytes: this.limits.maxOutputBytes };
        let handle;
    ...
    async installed(executable, signal) {
    ...
    engineRuntime() {
        if (this.limits.platform === 'darwin')
            return this.limits.engine.vmCli;
        if (this.limits.platform === 'linux')
            return this.limits.cli;
        return undefined;
    }
    async engineStatus(signal) {
    ...
    async installEngine(signal) {
        ...
        if (this.limits.platform !== 'darwin') {
            ...
            throw new DockerError(`installing a container runtime on ${this.limits.platform} is the operator's step; install docker with the distribution's package manager`, 'DOCKER_ENGINE_UNMANAGEABLE');
        }
        ...
        for (const formula of [this.limits.cli, runtime]) {
            const outcome = await this.tool([this.limits.engine.macInstaller, 'install', formula], this.limits.engine.installTimeoutMs, signal);
    ...
    async control(request, signal) {
        ...
        if (settled === undefined) {
            throw new DockerError(`container "${request.container}" is gone after ${request.action}`, 'DOCKER_NOT_FOUND');
        }
        return settled;
    }
    ...
    async list(request, signal) {
        ...
        if (request.project !== undefined)
            args.push('--filter', `label=${PROJECT_LABEL}=${request.project}`);
     ...
     */
    static projectOf(request, containers) {
    ...
    async composeUp(request, signal) {
        ...
        const containers = await this.list(request.project === undefined ? { all: true } : { all: true, project: request.project }, signal);
