 ...
 */
export function fail(failure) { throw new RemoteError(failure.code, failure.message, failure.details); }
...
export const DOCKER_SELECTION_CODES = new Set([
 ...
 */
export function dockerError(error) {
 ...
 */
export function dockerComposeError(error) {
    if (error instanceof DockerError && !DOCKER_SELECTION_CODES.has(error.code)) {
 ...
 */
export function dockerContainerEntry(container) {
    return {
        id: container.id,
        name: container.name,
 ...
 */
export function dockerImageEntry(image) {
    return {
        id: image.id,
        tags: [...image.tags],
        size: image.size,
 ...
 */
export function isComposeFileName(name) {
 ...
 */
export function projectSkillPath(path) {
    const skill = /^skills\/([a-z0-9-]+)\/(.+)$/.exec(path);
    if (skill !== null)
        return `.dsh/skills/${skill[1]}/${skill[2]}`;
    const command = /^commands\/([a-z0-9-]+\.md)$/.exec(path);
    if (command !== null)
        return `.dsh/skills/${command[1]}`;
    return undefined;
}
/**
 ...
 */
export function treadmillAbsent() {
    return {
 ...
 */
export function treadmillError(error) {
    if (remoteErrorOf(error))
        throw error;
...
export function editorError(error) {
    ...
    if (code === 'FS_STALE_VERSION') {
        return { code: 'editor-stale', message: error.message, details: {} };
    }
    if (code === 'FS_SANDBOX_DENIED' || code === 'FS_PERMISSION_DENIED') {
        return { code: 'editor-denied', message: error.message, details: {} };
    }
    if (code === 'FS_NOT_TEXT') {
        return { code: 'editor-not-text', message: error.message, details: {} };
    }
    if (code === 'FS_TOO_LARGE') {
        return { code: 'editor-too-large', message: error.message, details: {} };
    }
 ...
 */
export function editorPolicy(ctx, sessionId) {
 ...
 */
export async function editorRoot(ctx, fs, cwd, signal) {
    if (cwd !== undefined && cwd !== '')
        return fs.resolve(cwd, { signal });
 ...
 */
export async function editorTarget(fs, root, path, signal) {
 ...
 */
export function composeCrumbs(target) {
    const crumbs = [];
    let current = target;
    for (;;) {
 ...
 */
export async function composeLevel(target, maxEntries) {
    const directories = [];
    const files = [];
    let seen = 0;
    ...
    for await (const dirent of level) {
        ...
        if (!isDirectory && !isComposeFileName(dirent.name))
            continue;
        seen += 1;
        if (seen > maxEntries)
            continue;
 ...
 */
export async function projectTable(ctx, sessionId, signal) {
    const fs = ctx.get('fs');
    if (sessionId === undefined || fs === undefined)
        return undefined;
