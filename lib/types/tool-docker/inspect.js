...
function containerLine(container) {
 ...
 */
export function formatContainers(containers) {
    if (containers.length === 0)
        return 'No containers matched.';
 ...
 * @returns the plain JSON row the output schema declares.
 */
export function imageRow(image) {
 ...
 */
export function applyDockerInspectTools(ctx, timeoutMs, maxLogChars) {
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => presentListCall(args.project === undefined ? 'docker ps' : `docker ps — ${args.project}`),
    }));
    ctx.tools.register(defineTool({
        name: 'docker_images',
    ...
    ctx.tools.register(defineTool({
        ...
        isConcurrencySafe: () => true,
        async execute(args, exec) {
            if (args.container.trim().length === 0)
        ...
        presentCall: (args) => presentListCall(`docker logs ${args.container}`),
    }));
}
