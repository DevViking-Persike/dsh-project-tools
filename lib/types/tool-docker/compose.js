...
import { containerRow } from "./inspect.js";
/**
...
export function parseComposeArgs(args) {
    if (args.file.trim().length === 0)
        throw new Error('file must be a non-empty compose file path');
    if (args.services === undefined)
        return undefined;
    if (args.services.length === 0)
        throw new Error('services must contain at least one service when provided');
 ...
 */
export function formatComposeOutput(result) {
 ...
 */
export function capOutput(output, maxChars) {
...
function presentComposeCall(verb, args) {
...
function presentComposeResult(result) {
    const block = result.content.length === 1 ? result.content[0] : undefined;
    if (block === undefined || block.type !== 'text' || result.isError)
        return undefined;
 ...
 */
function composeValue(result, maxChars) {
    return {
 ...
 */
export function applyDockerComposeTools(ctx, timeoutMs, maxOutputChars) {
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => presentComposeCall('up', args),
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => presentComposeCall('down', args),
        presentResult: (_args, result) => presentComposeResult(result),
    }));
}
